...
<html>
    <head>
        <title>User Post Ranking like</title>
        <!-- Add Bootstrap CSS -->
    ...
    <body> 
        ...
        <div class="container" style="width: 500px">
            ...
            <canvas id="postChartlike"></canvas>
        </div>

        <!-- Add Bootstrap JS -->
        <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>

        <c:set var="listToplike" value="${requestScope.listUserHaveLikeByRank}"/> <!-- Ensure that the 'listTop' variable is set -->

        <script>
            document.addEventListener('DOMContentLoaded', () => {
            <%-- Get data from the data attribute --%>
            const data = [
            <c:forEach items="${listToplike}" var="userPost" varStatus="loop">
            {
            ...
            const ctx = document.getElementById('postChartlike').getContext('2d');
                    new Chart(ctx, {
                    type: 'bar',
                            data: {
                            labels: users,
                                    datasets: [{
                                    label: 'Thành viên có nhiều lượt thích',
                                            data: posts,
                                            backgroundColor: 'rgb(241, 148, 138, 0.6)',
                                            borderColor: 'rgb(236, 112, 99, 1)',
                                            borderWidth: 0.5,
